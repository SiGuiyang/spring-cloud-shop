...
public class CouponService implements IService<List<CouponResponse>> {
    ...
    public Response<List<CouponResponse>> doService(BaseDTO dto) {
        ...
        discountCoupons.forEach(discountCoupon -> {
            ...
            DiscountCouponTemplate discountCouponTemplate = discountCouponTemplateMapper.selectById(discountCoupon.getTemplateId());
            // 判断是否过期
            long beginTime = discountCouponTemplate.getBeginTime().getTime();
            long endTime = discountCouponTemplate.getEndTime().getTime();
